...
const ItemCardLarge: React.FC<ItemCardLargeProps> = ({ item }) => {
	return <div className={styles.itemcardlarge}>
		<img src={item.images.featured} alt={item.name} className={`hidden sm:inline-block w-[350px] h-full object-cover object-center ${item.rarity}`} />
		<img src={item.images.icon} alt={item.name} className={`sm:hidden w-[350px] h-full object-cover object-center ${item.rarity}`} />
		<div className={`absolute -rotate-2 h-5 w-96 transform -translate-x-8 bottom-20 shadow-lg ${item.rarity}`} />
