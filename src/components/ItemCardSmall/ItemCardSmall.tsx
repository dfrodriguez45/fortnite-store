...
const ItemCardSmall: React.FC<ItemCardSmallProps> = ({ item }) => {
	return <div className={styles.itemcardsmall}>
		<img src={item.images.icon} alt={item.name} className={`w-auto h-full object-cover object-center ${item.rarity}`} />
		<div className={`absolute -rotate-2 h-5 w-96 bottom-20 shadow-lg ${item.rarity}`} />
